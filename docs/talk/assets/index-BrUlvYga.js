...
On every local run, the smaller model reads one document, finds the facts, handles ambiguity, and decides whether the case needs a person. Both models reason about different parts of the problem at different frequencies.`},{id:`results-model-size`,type:`chart`,status:`working`,motion:`chart`,eyebrow:`Measured result 01`,title:`A 7.6 GB model scored [[93% on both tasks.]]`,image:`./charts/accuracy-by-size.svg`,alt:`Line chart comparing contract and inbox accuracy across five local models from 3.4 to 18 GB`,takeaway:`Same runbooks, same 46 synthetic documents, five models.`,notes:`I ran the same two workflows over the same 46 synthetic documents with five local models.
...
The shape matters more than one score. Inbox accuracy stays fairly flat across model sizes because it is classification into short lists. Contract accuracy rises sharply because the model has to connect clauses and calculate dates. A model can be good enough for one job and clearly insufficient for another.`},{id:`results-runbook-quality`,type:`chart`,status:`working`,motion:`chart`,eyebrow:`Measured result 02`,title:`For inbox sorting, instructions were worth [[38 points.]]`,image:`./charts/runbook-ladder.svg`,alt:`Two-panel line chart showing accuracy as progressively more complete instructions are added for inbox and contract triage`,takeaway:`Inbox: instructions +38 points, model upgrade +2. Contracts depend more on model capability.`,notes:`I then held the model, documents, and code fixed and changed only the completeness of the runbook.

On inbox sorting, the same model moved from 55 percent with a bare schema to 93 percent with the complete instructions. That is a 38 point gain. Upgrading to a larger model on the same task was worth only 2 points.

Contracts behave differently. Better instructions added 15.6 points, while the model upgrade added 18.4. That task needs more clause reading and date arithmetic. Instruction quality and model capability contribute differently depending on the work.`},{id:`results-redaction-gate`,type:`chart`,status:`working`,motion:`chart`,eyebrow:`Measured result 03`,title:`All five models removed [[every planted identifier.]]`,image:`./charts/redaction-gate.svg`,alt:`Matrix showing that zero of ten planted direct identifiers survived redaction for all five models`,takeaway:`0 of 10 direct identifiers survived the deterministic scan on any model.`,notes:`The privacy test used ten direct identifiers planted in the synthetic documents: names, addresses, phone numbers, email addresses, and account identifiers.
...
`,`<br />`)}</aside>`,ks=e=>`
  ...
  </div>`),Hs={title:Ls,question:Rs,journey:e=>zs(e,!1),local:e=>zs(e,!0),scale:Bs,cards:Vs,chart:e=>As(e,`<div class="chart-layout">
    <div class="chart-header">
      ${js(e.eyebrow)}
      ${Ms(e.title,`display display--chart`)}
    </div>
    <div class="chart-frame">
      <img class="chart-image js-chart" src="${$(e.image)}" alt="${$(e.alt)}" decoding="async" loading="eager" />
    </div>
    <p class="chart-takeaway js-subtitle">${$(e.takeaway)}</p>
  </div>`),statement:e=>As(e,`<div class="statement-layout">
    ${js(e.eyebrow)}
